...
router.get("/courses", requireClientAuth, async (req: any, res): Promise<void> => {
  ...
  const progress = await db.select().from(courseProgressTable).where(eq(courseProgressTable.profileId, profileId));
  ...
  const result = courses.map((course) => {
    ...
    const progressPercent = courseLessons.length > 0
      ...
      : 0;
    return { ...course, lessonCount: courseLessons.length, progressPercent };
  });

  res.json(ListCoursesResponse.parse(result));
});

router.get("/courses/:courseId", requireClientAuth, async (req: any, res): Promise<void> => {
  const profileId = req.clientUser.id;
  ...
  const progress = await db.select().from(courseProgressTable)
    .where(and(
      eq(courseProgressTable.profileId, profileId),
...
router.patch("/courses/:courseId/progress", requireClientAuth, async (req: any, res): Promise<void> => {
  const profileId = req.clientUser.id;
  ...
  const allProgress = await db.select().from(courseProgressTable).where(
    and(eq(courseProgressTable.profileId, profileId), eq(courseProgressTable.courseId, params.data.courseId))
  );
