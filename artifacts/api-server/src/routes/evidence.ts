...
// ─── POST /api/evidence/drive-folder ─────────────────────────────────────────
// Lets a client register a Google Drive folder URL for a given criterion.
// The folder is validated for accessibility before saving.

router.post("/evidence/drive-folder", requireClientAuth, async (req: any, res) => {
  const userId: number = req.clientUser.id;
  const { criteria_id, folder_url } = req.body as {
    criteria_id?: string;
    folder_url?: string;
  };

  if (!criteria_id || !folder_url) {
    res.status(400).json({ error: "criteria_id and folder_url are required" });
    return;
  }

  // Ensure intake is complete and visa path is known
  const [intake] = await db
    .select({ visaPath: readinessIntakeTable.visaPath })
    .from(readinessIntakeTable)
    .where(eq(readinessIntakeTable.profileId, userId))
  ...
  if (!criteriaEntry) {
    res.status(400).json({ error: `criteria_id "${criteria_id}" is not valid for your visa path` });
    return;
  }

  // Parse folder ID from a Google Drive URL such as:
  //   https://drive.google.com/drive/folders/FOLDER_ID
  //   https://drive.google.com/drive/folders/FOLDER_ID?usp=sharing
  const folderIdMatch = folder_url.match(/\/folders\/([a-zA-Z0-9_-]+)/);
  if (!folderIdMatch) {
    res.status(400).json({
      error: "Invalid Google Drive folder URL. Expected a URL like https://drive.google.com/drive/folders/FOLDER_ID",
    });
    return;
  }
  const driveFolderId = folderIdMatch[1];

  // Validate folder is accessible via the service account
  try {
    const drive = getDriveClient();
    await listFolderFiles(drive, driveFolderId);
  } catch {
    res.status(400).json({
      error:
        "Could not access this Google Drive folder. Please make sure it is shared with the Pinnacle service account.",
    });
    return;
  }

  // Upsert: update if already exists for this (profile, criteria), otherwise insert
  ...
  if (existing) {
    [folderRow] = await db
      ...
      .set({ driveFolderId, driveFolderUrl: folder_url, folderName: criteriaEntry.folderName })
    ...
    [folderRow] = await db
      ...
      .values({
        ...
        visaPath,
        folderName: criteriaEntry.folderName,
        driveFolderId,
